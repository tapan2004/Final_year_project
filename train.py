...
DATA_CSV = r"D:\TapanProgramming\finalYearProject\english_to_bangla.csv"
...
def calculate_bleu_score(predictions, references):
    ...
    for pred, ref in zip(predictions, references):
        ...
        if len(pred_words) > 0:
            score = len(pred_words.intersection(ref_words)) / len(pred_words)
            total_score += score
...
try:
    ...
    df = pd.read_csv(DATA_CSV, usecols=["en", "bn"]).dropna()
    if len(df) == 0:
...
df = df[(df["en"].str.len() > 3) & (df["bn"].str.len() > 3)]
...
en_tokenizer.build_vocab(df["en"].tolist())
bn_tokenizer.build_vocab(df["bn"].tolist())
...
class SimpleTranslationDataset(Dataset):
    ...
    def __getitem__(self, i):
        row = self.df.iloc[i]
        src = torch.tensor(self.encode_sequence(row["en"], self.en_tokenizer), dtype=torch.long)
        tgt = torch.tensor(self.encode_sequence(row["bn"], self.bn_tokenizer), dtype=torch.long)
        return src, tgt
...
print(f"Bangla vocab size: {BN_VOCAB_SIZE}")
...
for epoch in range(1, EPOCHS + 1):
    ...
    try:
        ...
        if "out of memory" in str(e):
            print(f"CUDA out of memory at epoch {epoch}. Try reducing batch size.")
            if DEVICE == "cuda":
                torch.cuda.empty_cache()
            break
        else:
...
def load_model(model_path='best_simple_model.pth'):
    try:
        checkpoint = torch.load(model_path, map_location=DEVICE)
        config = checkpoint['config']
        encoder = Encoder(config['EN_VOCAB_SIZE'], config['EMBED'], config['HIDDEN']).to(DEVICE)
        decoder = Decoder(config['BN_VOCAB_SIZE'], config['EMBED'], config['HIDDEN']).to(DEVICE)
        model = Seq2Seq(encoder, decoder).to(DEVICE)
        model.load_state_dict(checkpoint['model_state_dict'])
        return model, checkpoint['en_tokenizer'], checkpoint['bn_tokenizer']
    except Exception as e:
